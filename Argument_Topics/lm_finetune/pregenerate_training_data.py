...
import json
import collections
from constants import BERT_PRETRAINED_MODEL, ARGUMENT_TOPICS_PRETRAIN_DATA_DIR, MAX_ARGUMENT_SEQ_LENGTH, \
    ARGUMENT_TOPICS_DATASETS_DIR, ARGUMENT_TOPICS_DOMAIN_TREAT_CONTROL_MAP_FILE, ARGUMENT_DOMAINS
...
def main():
    ...
    parser.add_argument("--max_seq_len", type=int, default=MAX_ARGUMENT_SEQ_LENGTH)
...
def generate_data_for_domain(args, domain):
    ...
    treatment_column = f"topic_{treatment_topic}_bin"
    control_column = f"topic_{control_topic}_bin"

    with DocumentDatabase(reduce_memory=args.reduce_memory) as docs:
        print(f"\nGenerating data for domain: {domain}")
        output_dir = Path(ARGUMENT_TOPICS_PRETRAIN_DATA_DIR) / domain
        output_dir.mkdir(exist_ok=True, parents=True)
        unique_ids, reviews, treatment_labels, control_labels = list(), list(), list(), list()
        for dataset in ("train", "dev"):
            DATASET_FILE = f"{ARGUMENT_TOPICS_DATASETS_DIR}/all_except_{domain}/topics_{dataset}.csv"
            df = pd.read_csv(DATASET_FILE, header=0, encoding='utf-8', usecols=["id", "sentence", treatment_column, control_column]).set_index(keys="id", drop=False).sort_index()
            df = df[df[treatment_column].notnull()]
            unique_ids += df["id"].tolist()
            reviews += df["sentence"].apply(tokenizer.tokenize).tolist()
