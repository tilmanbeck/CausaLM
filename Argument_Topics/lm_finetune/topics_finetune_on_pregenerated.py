...
from Argument_Topics.lm_finetune.pregenerate_training_data import EPOCHS
from Argument_Topics.lm_finetune.bert_topics_finetune import BertForTopicTreatControlPreTraining, \
    BertForTopicTreatPreTraining
from BERT.bert_text_dataset import BertTextDataset
from utils import init_logger
import json
from constants import RANDOM_SEED, ARGUMENT_TOPICS_PRETRAIN_DIR, BERT_PRETRAINED_MODEL, NUM_CPU, \
    ARGUMENT_TOPICS_PRETRAIN_DATA_DIR, ARGUMENT_TOPICS_PRETRAIN_IXT_DIR, ARGUMENT_DOMAINS, ARGUMENT_ID2INT
...
logger = init_logger("Argument-Topics-pretraining", f"{ARGUMENT_TOPICS_PRETRAIN_DIR}")
...
def convert_example_to_features(example, tokenizer, max_seq_length):
    ...
    unique_id = int(id2int[example["unique_id"]])
...
def main():
    ...
    args.pregenerated_data = Path(ARGUMENT_TOPICS_PRETRAIN_DATA_DIR) / args.domain
    if args.control:
        args.output_dir = Path(ARGUMENT_TOPICS_PRETRAIN_IXT_DIR) / args.domain / "model_control"
    else:
        args.output_dir = Path(ARGUMENT_TOPICS_PRETRAIN_IXT_DIR) / args.domain / "model"
