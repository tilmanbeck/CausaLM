...
from constants import ARGUMENT_EXPERIMENTS_DIR, MAX_ARGUMENT_SEQ_LENGTH, ARGUMENT_DOMAINS, \
    ARGUMENT_TOPICS_DOMAIN_TREAT_CONTROL_MAP_FILE, ARGUMENT_TOPICS_DATASETS_DIR, ARGUMENT_TOPICS_PRETRAIN_IXT_DIR
from pytorch_lightning import Trainer
from BERT.bert_text_classifier import LightningBertPretrainedClassifier, LightningHyperparameters
from Argument_Topics.pipeline.predict import print_final_metrics, predict_models
...
def main():
    ...
    parser.add_argument("--domain", type=str, default="abortion", choices=ARGUMENT_DOMAINS),
...
def train_models_unit(hparams: Dict, task, group, pretrained_control):
    label_size = 2
    if task == "Argument":
        label_column = f"{task.lower()}_label_bin" # here we have to add the bin
    elif task == "ITT":
    ...
    if hparams["bert_params"]["bert_state_dict"]:
        if pretrained_control:
            ...
            hparams["bert_params"]["name"] = f"{task}_{group}_topic_{hparams['treatment_column'].split('_')[1]}_treated"
    else:
        hparams["bert_params"]["name"] = f"{task}_{group}"
    OUTPUT_DIR = f"{ARGUMENT_EXPERIMENTS_DIR}/{hparams['treatment']}/{hparams['domain']}/{hparams['bert_params']['name']}"
...
def train_models(hparams: Dict, group: str, pretrained_epoch: int, pretrained_control: bool):
    ...
    argument_model = train_models_unit(hparams, "Argument", group, pretrained_control)
    ...
    predict_models(hparams['treatment'], hparams['domain'], group, pretrained_epoch, pretrained_control,
                   argument_model, itt_model, itc_model, hparams["bert_params"]["bert_state_dict"])
...
def train_all_models(args, domain: str):
    ...
    hparams = {
        "treatment": treatment,
        "data_path": f"{ARGUMENT_TOPICS_DATASETS_DIR}/all_except_{domain}", # here we adapted path
        "domain": domain,
        "treatment_column": f"{treatment_topic}_bin",
        "control_column": f"{control_topic}_bin",
        "text_column": "sentence",
        "label_column": "argument_label_bin",
        "epochs": args.epochs,
        "accumulate": ACCUMULATE,
        "max_seq_len": MAX_ARGUMENT_SEQ_LENGTH,
        "bert_params": {
            "batch_size": args.batch_size,
            "dropout": DROPOUT,
            "bert_state_dict": None,
            "label_size": 2,
            "name": f"Argument_{args.group}"
        }
    }
