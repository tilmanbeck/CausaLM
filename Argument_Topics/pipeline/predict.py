...
from constants import ARGUMENT_TOPICS_DATASETS_DIR, ARGUMENT_EXPERIMENTS_DIR, \
    ARGUMENT_TOPICS_PRETRAIN_MLM_DIR, ARGUMENT_TOPICS_PRETRAIN_IXT_DIR, ARGUMENT_DOMAINS, \
    ARGUMENT_TOPICS_DOMAIN_TREAT_CONTROL_MAP_FILE
...
def main():
    ...
    parser.add_argument("--domain", type=str, default="books", choices=ARGUMENT_DOMAINS),
...
def predict_models_unit(task, treatment, domain, trained_group, group, model_ckpt,
    ...
    label_size = 2
    if task == "Argument":
        label_column = f"{task.lower()}_label_bin" # binarized label
    elif "ITT" in task:
    ...
    if not model_ckpt:
        ...
        models_dir = f"{ARGUMENT_EXPERIMENTS_DIR}/{hparams['treatment']}/{hparams['domain']}/{model_name}/lightning_logs/*"
    ...
    hparams["bert_params"]["bert_state_dict"] = f"{ARGUMENT_TOPICS_PRETRAIN_MLM_DIR}/{domain}/model/epoch_{pretrained_epoch}/pytorch_model.bin"
    ...
    if not bert_state_dict:
        ...
        hparams["bert_params"]["bert_state_dict"] = f"{ARGUMENT_TOPICS_PRETRAIN_IXT_DIR}/{state_dict_dir}/pytorch_model.bin"
...
def predict_models(treatment="topics", domain="books", trained_group="F", pretrained_epoch=None, pretrained_control=None,
                   argument_model_ckpt=None, itt_model_ckpt=None, itc_model_ckpt=None, bert_state_dict=None):
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    with open(ARGUMENT_TOPICS_DOMAIN_TREAT_CONTROL_MAP_FILE, "r") as jsonfile:
    ...
    hparams = {
        ...
        "data_path": ARGUMENT_TOPICS_DATASETS_DIR,
        "bert_params": {
            "bert_state_dict": bert_state_dict
        }
    }
    OUTPUT_DIR = f"{ARGUMENT_EXPERIMENTS_DIR}/{treatment}/{domain}/COMPARE"
    ...
    for task, model in zip(("Argument", "CONTROL_ITT", "CONTROL_ICT"),
                           (argument_model_ckpt, itt_model_ckpt, itc_model_ckpt)):
        for group in ("F",):
...
def predict_all_models(args, domain: str):
    ...
    if args.pretrained_control:
        pretrained_treated_model_dir = f"{ARGUMENT_TOPICS_PRETRAIN_IXT_DIR}/{domain}/model_control"
        trained_group = f"{args.trained_group}_{treatment_topic}_treated_{control_topic}_controlled"
    else:
        pretrained_treated_model_dir = f"{ARGUMENT_TOPICS_PRETRAIN_IXT_DIR}/{domain}/model"
